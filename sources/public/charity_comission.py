...
def charities_by_la(data):
    ...
    df = df.groupby('utla_name').count().sort_values('count', ascending=False).reset_index()
    ...
    logging.warning(f"No matches for {[name for name in df.loc[no_match, 'utla_name_cc']]}")
...
def normalise_charities_utla(data):
    ...
    utla_pop = pd.merge(
        pop, 
        lkp[['la_code', 'utla_code', 'utla_name']], 
        on='la_code'
    ).groupby(
        ['utla_code', 'utla_name'],
